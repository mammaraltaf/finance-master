            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="categoryForm" method="POST" action="{{route('admin.type-of-expense-post')}}"
...
{{--                  'role' => 'form' )) !!}--}}
                ...
                <form method="post" action="{{route('admin.delete-type-of-expense')}}">
                    @csrf
...
@section('script')
    ...
    <script type="text/javascript">
     ...
     $(document).ready(function() {
            ...
            $('#typeOfExpenseTable').css('zoom', zoomLevel + '%');
            }

        
            $('#typeOfExpenseTable').on('wheel', function(event) {
            if (event.ctrlKey) {
    ...
    <script type="text/javascript">
        ...
        $('body').on('click', '#typeOfExpenseEdit', function () {
            ...
            $.ajax({
                type: "GET",
                url: "{{url('/admin/edit-type-of-expense/')}}"+'/'+typeOfExpanse_id,
                success:function (response){
                    // console.log(response);
                    $('#id_software').val(response.id_software);
                    $('#name').val(response.name);
                    $('#accounting_id').val(response.accounting_id);
                    $('#companyFormEdit').attr('action',"{{url('/admin/edit-type-of-expense/')}}"+'/'+typeOfExpanse_id);
                }
            });
        });
