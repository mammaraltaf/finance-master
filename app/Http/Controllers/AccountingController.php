...
class AccountingController extends Controller
{
    ...
    public function printRequest($id)
        ...
        $pdf = PDF::loadView('templateRequest', compact('request', 'logs'));
...
//                    return response()->json(['success' => 'Bulk Requests Rejected successfully.']);
                ...
                $formattedExportData = $bogRows->map(function ($requestData) {
                    return [
                        ...
                        'amount_in_gel' => number_format($requestData->amount_in_gel, 2, '.', ''),
                    ];
                });
                ...
                foreach ($tbcRows as $requestData) {
                    ...
                    $xmlString .= '<ns1:AMOUNT>' . number_format($requestData->amount_in_gel, 2, '.', '') . '</ns1:AMOUNT>' . PHP_EOL;
